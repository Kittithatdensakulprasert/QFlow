...
func (r *notificationRepository) FindByUserID(userID uint, offset, limit int) ([]domain.Notification, error) {
...
func (r *notificationRepository) CountByUserID(userID uint) (int64, error) {
	var total int64
	err := r.db.Model(&domain.Notification{}).Where("user_id = ?", userID).Count(&total).Error
	return total, err
}
